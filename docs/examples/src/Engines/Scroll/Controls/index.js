...
var $author$project$Anim$Internal$Scroll$AnimState = function (a) {
	return {$: 'AnimState', a: a};
};
var $elm$core$Dict$RBEmpty_elm_builtin = {$: 'RBEmpty_elm_builtin'};
var $elm$core$Dict$empty = $elm$core$Dict$RBEmpty_elm_builtin;
var $author$project$Anim$Internal$Scroll$init = $author$project$Anim$Internal$Scroll$AnimState(
	{animations: $elm$core$Dict$empty, nextId: 1});
var $author$project$Anim$Engine$Scroll$init = $author$project$Anim$Internal$Scroll$init;
	...
	return _Utils_Tuple2(
		{
			...
			scrollState: $author$project$Anim$Engine$Scroll$init
		},
...
var $author$project$Anim$Internal$Scroll$DomQueriesCompleted = F4(
	function (a, b, c, d) {
		return {$: 'DomQueriesCompleted', a: a, b: b, c: c, d: d};
	});
var $author$project$Anim$Internal$Scroll$NoOp = {$: 'NoOp'};
...
var $author$project$Anim$Internal$Scroll$animate = F3(
	function (toMsg, _v0, buildAnimation) {
		var initAnimState = $author$project$Anim$Internal$Scroll$AnimState(
			{animations: $elm$core$Dict$empty, nextId: 1});
		var animBuilder = buildAnimation($author$project$Anim$Internal$Builder$init);
		var scrollTargets = $author$project$Anim$Internal$Builder$getScrollTargets(animBuilder);
		var domQueries = A2(
			$elm$core$List$indexedMap,
			F2(
				function (index, scrollTarget) {
					var targetType = $author$project$Anim$Internal$Properties$ScrollTarget$getTargetType(scrollTarget);
					var containerId = $author$project$Anim$Internal$Properties$ScrollTarget$getContainerId(scrollTarget);
					var animId = $elm$core$String$fromInt(index + 1);
					if (targetType.$ === 'Element') {
						var elementId = targetType.a;
						return (containerId === 'document') ? A2(
							$elm$core$Task$attempt,
							function (result) {
								if (result.$ === 'Ok') {
									var domResult = result.a;
									return toMsg(
										A4($author$project$Anim$Internal$Scroll$DomQueriesCompleted, animId, scrollTarget, animBuilder, domResult));
								} else {
									return toMsg($author$project$Anim$Internal$Scroll$NoOp);
								}
							},
							A3(
								$elm$core$Task$map2,
								F2(
									function (viewport, element) {
										return {
											containerElement: $elm$core$Maybe$Nothing,
											targetElement: $elm$core$Maybe$Just(element),
											viewport: viewport
										};
									}),
								$elm$browser$Browser$Dom$getViewport,
								$elm$browser$Browser$Dom$getElement(elementId))) : A2(
							$elm$core$Task$attempt,
							function (result) {
								if (result.$ === 'Ok') {
									var domResult = result.a;
							...
							A4(
								...
								F3(
									function (viewport, containerEl, element) {
										return {
											containerElement: $elm$core$Maybe$Just(containerEl),
											targetElement: $elm$core$Maybe$Just(element),
											viewport: viewport
										};
									}),
						...
						return A2(
							...
							A2(
								$elm$core$Task$map,
								function (viewport) {
									return {containerElement: $elm$core$Maybe$Nothing, targetElement: $elm$core$Maybe$Nothing, viewport: viewport};
								},
								(containerId === 'document') ? $elm$browser$Browser$Dom$getViewport : $elm$browser$Browser$Dom$getViewportOf(containerId)));
					}
				}),
			scrollTargets);
		return _Utils_Tuple2(
...
var $author$project$Anim$Internal$Scroll$containerIdMatches = F2(
	function (id, containerId) {
		if (containerId.$ === 'DocumentBody') {
			return (id === 'document') || (id === 'body');
		} else {
			var elementId = containerId.a;
			return _Utils_eq(id, elementId);
		}
	});
...
var $author$project$Anim$Internal$Scroll$pauseContainer = F2(
	function (containerId, _v0) {
		var animData = _v0.a;
		var updatedAnimations = A2(
			$elm$core$Dict$map,
			F2(
				function (_v1, anim) {
					return A2($author$project$Anim$Internal$Scroll$containerIdMatches, containerId, anim.config.containerId) ? _Utils_update(
						anim,
						{isPaused: true}) : anim;
				}),
			animData.animations);
		return $author$project$Anim$Internal$Scroll$AnimState(
			_Utils_update(
				animData,
				{animations: updatedAnimations}));
	});
var $author$project$Anim$Engine$Scroll$pauseContainer = $author$project$Anim$Internal$Scroll$pauseContainer;
...
var $author$project$Anim$Internal$Scroll$scrollToPosition = F4(
	function (toMsg, containerId, x, y) {
		if (containerId.$ === 'DocumentBody') {
			return A2(
				$elm$core$Task$attempt,
				function (_v1) {
			...
			return A2(
				...
				A3($elm$browser$Browser$Dom$setViewportOf, elementId, x, y));
		}
	});
var $author$project$Anim$Internal$Scroll$resetContainer = F3(
	function (toMsg, containerId, _v0) {
		...
		var _v1 = A3(
			$elm$core$Dict$foldl,
			F3(
				function (animId, anim, _v2) {
					var accAnims = _v2.a;
					var accCmds = _v2.b;
					if (A2($author$project$Anim$Internal$Scroll$containerIdMatches, containerId, anim.config.containerId)) {
						var updatedAnim = _Utils_update(
							anim,
							{currentX: anim.startX, currentY: anim.startY, delayComplete: anim.delayMs === 0.0, elapsedMs: 0.0, isPaused: true, progress: 0.0});
						var scrollCmd = A4($author$project$Anim$Internal$Scroll$scrollToPosition, toMsg, anim.config.containerId, anim.startX, anim.startY);
						return _Utils_Tuple2(
							A3($elm$core$Dict$insert, animId, updatedAnim, accAnims),
							A2($elm$core$List$cons, scrollCmd, accCmds));
					} else {
						return _Utils_Tuple2(
							A3($elm$core$Dict$insert, animId, anim, accAnims),
							accCmds);
					}
				}),
		...
		var scrollCmds = _v1.b;
		return _Utils_Tuple2(
			$author$project$Anim$Internal$Scroll$AnimState(
				_Utils_update(
					animData,
					{animations: updatedAnimations})),
			$elm$core$Platform$Cmd$batch(scrollCmds));
	});
			...
			F3(
				function (animId, anim, _v2) {
					...
					if (A2($author$project$Anim$Internal$Scroll$containerIdMatches, containerId, anim.config.containerId)) {
						var updatedAnim = _Utils_update(
							anim,
							{currentX: anim.startX, currentY: anim.startY, delayComplete: anim.delayMs === 0.0, elapsedMs: 0.0, isPaused: false, progress: 0.0});
						var scrollCmd = A4($author$project$Anim$Internal$Scroll$scrollToPosition, toMsg, anim.config.containerId, anim.startX, anim.startY);
						return _Utils_Tuple2(
							A3($elm$core$Dict$insert, animId, updatedAnim, accAnims),
							A2($elm$core$List$cons, scrollCmd, accCmds));
...
var $author$project$Anim$Engine$Scroll$restartContainer = $author$project$Anim$Internal$Scroll$restartContainer;
var $author$project$Anim$Internal$Scroll$resumeContainer = F2(
	function (containerId, _v0) {
		var animData = _v0.a;
		var updatedAnimations = A2(
			$elm$core$Dict$map,
...
var $author$project$Anim$Engine$Scroll$resumeContainer = $author$project$Anim$Internal$Scroll$resumeContainer;
...
var $author$project$Engines$Scroll$Controls$Main$scrollAnimation = A2(
	...
	A2(
		...
		A2(
			$elm$core$Basics$composeR,
			$author$project$Anim$Engine$Scroll$duration(3000),
			A2(
				$elm$core$Basics$composeR,
				$author$project$Anim$Engine$Scroll$easing($author$project$Anim$Extra$Easing$EaseInOut),
		...
		var _v1 = A3(
			...
			F3(
				function (animId, anim, _v2) {
					...
					var accCmds = _v2.b;
					if (A2($author$project$Anim$Internal$Scroll$containerIdMatches, containerId, anim.config.containerId)) {
						var scrollCmd = A4($author$project$Anim$Internal$Scroll$scrollToPosition, toMsg, anim.config.containerId, anim.config.targetX, anim.config.targetY);
						return _Utils_Tuple2(
							accAnims,
							A2($elm$core$List$cons, scrollCmd, accCmds));
					} else {
...
var $author$project$Anim$Engine$Scroll$stopContainer = $author$project$Anim$Internal$Scroll$stopContainer;
...
var $author$project$Anim$Internal$Scroll$calculateDistance = F5(
...
var $author$project$Anim$Internal$Scroll$ElementId = function (a) {
	return {$: 'ElementId', a: a};
};
var $author$project$Anim$Internal$Scroll$Both = {$: 'Both'};
var $author$project$Anim$Internal$Scroll$X = {$: 'X'};
var $author$project$Anim$Internal$Scroll$Y = {$: 'Y'};
var $author$project$Anim$Internal$Scroll$convertAxis = function (axis) {
	switch (axis.$) {
		case 'X':
			return $author$project$Anim$Internal$Scroll$X;
		case 'Y':
			return $author$project$Anim$Internal$Scroll$Y;
		default:
			return $author$project$Anim$Internal$Scroll$Both;
	}
};
var $author$project$Anim$Internal$Properties$ScrollTarget$getAxis = function (_v0) {
	var data = _v0.a;
	return data.axis;
};
...
var $author$project$Anim$Extra$Easing$QuintOut = {$: 'QuintOut'};
var $author$project$Anim$Internal$Builder$getEasingWithDefault = function (_v0) {
	var data = _v0.a;
	return A2($elm$core$Maybe$withDefault, $author$project$Anim$Extra$Easing$QuintOut, data.globalEasing);
};
var $author$project$Anim$Internal$Properties$ScrollTarget$getTargetX = function (_v0) {
	var data = _v0.a;
	var _v1 = data.target;
	switch (_v1.$) {
		case 'Coordinates':
...
var $author$project$Anim$Internal$Properties$ScrollTarget$getTargetY = function (_v0) {
	var data = _v0.a;
	var _v1 = data.target;
	switch (_v1.$) {
		case 'Coordinates':
...
var $author$project$Anim$Internal$Scroll$createScrollAnimationConfig = F2(
	function (animBuilder, scrollTarget) {
		return {
			axis: $author$project$Anim$Internal$Scroll$convertAxis(
				$author$project$Anim$Internal$Properties$ScrollTarget$getAxis(scrollTarget)),
			containerId: $author$project$Anim$Internal$Scroll$ElementId(
				$author$project$Anim$Internal$Properties$ScrollTarget$getContainerId(scrollTarget)),
			delay: $author$project$Anim$Internal$Builder$getDelayWithDefault(animBuilder),
			easing: $author$project$Anim$Internal$Builder$getEasingWithDefault(animBuilder),
			targetX: $author$project$Anim$Internal$Properties$ScrollTarget$getTargetX(scrollTarget),
			targetY: $author$project$Anim$Internal$Properties$ScrollTarget$getTargetY(scrollTarget),
			timeSpec: $author$project$Anim$Internal$Builder$getTimeSpec(animBuilder)
		};
	});
...
var $author$project$Anim$Internal$Scroll$speedToDuration = F2(
	function (speedPxPerSec, distance) {
		return (speedPxPerSec <= 0) ? 400 : $elm$core$Basics$round((distance * 1000) / speedPxPerSec);
	});
var $author$project$Anim$Internal$Scroll$createScrollAnimationFromDom = F4(
	function (animBuilder, scrollTarget, domResult, element) {
		var viewport = domResult.viewport;
		var startPosition = {x: viewport.viewport.x, y: viewport.viewport.y};
		var elementContentPosition = function () {
			var _v2 = domResult.containerElement;
			if (_v2.$ === 'Just') {
				var containerEl = _v2.a;
				return {x: (element.element.x - containerEl.element.x) + viewport.viewport.x, y: (element.element.y - containerEl.element.y) + viewport.viewport.y};
			} else {
				return {x: element.element.x + viewport.viewport.x, y: element.element.y + viewport.viewport.y};
			}
		}();
		var targetPosition = function () {
			var _v1 = $author$project$Anim$Internal$Properties$ScrollTarget$getTargetType(scrollTarget);
			switch (_v1.$) {
				case 'Element':
					return {x: elementContentPosition.x, y: elementContentPosition.y};
				case 'Coordinates':
					var x = _v1.a;
					var y = _v1.b;
					return {x: x, y: y};
		...
		var delayMs = A2(
			$elm$core$Maybe$withDefault,
			0,
			$author$project$Anim$Internal$Builder$getDelay(animBuilder));
		var clampedTarget = {
			x: A3($elm$core$Basics$clamp, 0, viewport.scene.width - viewport.viewport.width, targetPosition.x),
			y: A3($elm$core$Basics$clamp, 0, viewport.scene.height - viewport.viewport.height, targetPosition.y)
		};
		var baseConfig = A2($author$project$Anim$Internal$Scroll$createScrollAnimationConfig, animBuilder, scrollTarget);
		var config = _Utils_update(
			baseConfig,
			{targetX: clampedTarget.x, targetY: clampedTarget.y});
		var distance = A5($author$project$Anim$Internal$Scroll$calculateDistance, config.axis, startPosition.x, startPosition.y, clampedTarget.x, clampedTarget.y);
		var actualDuration = function () {
			var _v0 = $author$project$Anim$Internal$Builder$getTimeSpec(animBuilder);
			if (_v0.$ === 'Duration') {
				var ms = _v0.a;
				return ms;
			} else {
				var pxPerSec = _v0.a;
				return A2($author$project$Anim$Internal$Scroll$speedToDuration, pxPerSec, distance);
			}
		}();
		return {config: config, currentX: startPosition.x, currentY: startPosition.y, delayComplete: delayMs === 0.0, delayMs: delayMs, durationMs: actualDuration, elapsedMs: 0.0, isPaused: false, progress: 0.0, startX: startPosition.x, startY: startPosition.y};
	});
var $author$project$Anim$Internal$Scroll$createScrollAnimationFromViewport = F3(
	function (animBuilder, scrollTarget, viewport) {
		var targetPosition = {
			x: $author$project$Anim$Internal$Properties$ScrollTarget$getTargetX(scrollTarget),
			y: $author$project$Anim$Internal$Properties$ScrollTarget$getTargetY(scrollTarget)
		};
		var startPosition = {x: viewport.viewport.x, y: viewport.viewport.y};
		var delayMs = A2(
			$elm$core$Maybe$withDefault,
			0,
			$author$project$Anim$Internal$Builder$getDelay(animBuilder));
		var config = A2($author$project$Anim$Internal$Scroll$createScrollAnimationConfig, animBuilder, scrollTarget);
		var distance = A5($author$project$Anim$Internal$Scroll$calculateDistance, config.axis, startPosition.x, startPosition.y, targetPosition.x, targetPosition.y);
		var actualDuration = function () {
			var _v0 = $author$project$Anim$Internal$Builder$getTimeSpec(animBuilder);
			if (_v0.$ === 'Duration') {
				var ms = _v0.a;
...
var $author$project$Anim$Internal$Scroll$updateScrollAnimation = F2(
	function (deltaMs, animation) {
		if (!animation.delayComplete) {
			var newElapsedMs = animation.elapsedMs + deltaMs;
			return (_Utils_cmp(newElapsedMs, animation.delayMs) > -1) ? _Utils_update(
				animation,
				{delayComplete: true, elapsedMs: 0.0}) : _Utils_update(
				animation,
				{elapsedMs: newElapsedMs});
		} else {
			var newElapsedMs = animation.elapsedMs + deltaMs;
			var progress = A2($elm$core$Basics$min, 1.0, newElapsedMs / animation.durationMs);
			var easingFunction = A2($author$project$Anim$Internal$Easing$toFunction, animation.durationMs, animation.config.easing);
			var easedProgress = easingFunction(progress);
			var newCurrentX = function () {
				var _v1 = animation.config.axis;
				if (_v1.$ === 'Y') {
					return animation.startX;
				} else {
					return animation.startX + ((animation.config.targetX - animation.startX) * easedProgress);
				}
			}();
			var newCurrentY = function () {
				var _v0 = animation.config.axis;
				if (_v0.$ === 'X') {
					return animation.startY;
				} else {
					return animation.startY + ((animation.config.targetY - animation.startY) * easedProgress);
				}
			}();
			return _Utils_update(
				animation,
				{currentX: newCurrentX, currentY: newCurrentY, elapsedMs: newElapsedMs, progress: progress});
		}
	});
var $author$project$Anim$Internal$Scroll$update = F3(
	function (toMsg, msg, _v0) {
		var animData = _v0.a;
		switch (msg.$) {
			case 'AnimationFrame':
				var deltaMs = msg.a;
					...
					F3(
						function (animId, anim, _v3) {
							...
							if (anim.isPaused) {
								...
								var updatedAnim = A2($author$project$Anim$Internal$Scroll$updateScrollAnimation, deltaMs, anim);
								var scrollCmd = function () {
									var _v4 = updatedAnim.config.containerId;
									if (_v4.$ === 'DocumentBody') {
										return A2(
											$elm$core$Task$attempt,
											function (_v5) {
											...
											A2($elm$browser$Browser$Dom$setViewport, updatedAnim.currentX, updatedAnim.currentY));
									} else {
										var containerId = _v4.a;
										return A2(
											$elm$core$Task$attempt,
											function (_v6) {
												return toMsg($author$project$Anim$Internal$Scroll$NoOp);
											},
											A3($elm$browser$Browser$Dom$setViewportOf, containerId, updatedAnim.currentX, updatedAnim.currentY));
									}
								}();
								if (updatedAnim.progress < 1.0) {
									return _Utils_Tuple2(
										A3($elm$core$Dict$insert, animId, updatedAnim, accAnims),
										A2($elm$core$List$cons, scrollCmd, accCmds));
								} else {
									var completedAnim = _Utils_update(
										updatedAnim,
										{isPaused: true});
									return _Utils_Tuple2(
										A3($elm$core$Dict$insert, animId, completedAnim, accAnims),
										A2($elm$core$List$cons, scrollCmd, accCmds));
								}
							}
						}),
			...
			case 'DomQueriesCompleted':
				var animId = msg.a;
				var scrollTarget = msg.b;
				var animBuilder = msg.c;
				var domResult = msg.d;
				var animation = function () {
					var _v7 = domResult.targetElement;
					if (_v7.$ === 'Just') {
						var element = _v7.a;
						return A4($author$project$Anim$Internal$Scroll$createScrollAnimationFromDom, animBuilder, scrollTarget, domResult, element);
					} else {
			...
			default:
				return _Utils_Tuple2(
					$author$project$Anim$Internal$Scroll$AnimState(animData),
					$elm$core$Platform$Cmd$none);
		}
	});
...
var $author$project$Engines$Scroll$Controls$Main$targetSection = A2(
	...
	_List_fromArray(
		[
			...
			A3($mdgriffith$elm_ui$Element$rgb255, 255, 243, 224)),
			$mdgriffith$elm_ui$Element$Border$rounded(8),
		...
		[
			...
			_List_fromArray(
				[
					...
					A3($mdgriffith$elm_ui$Element$rgb255, 230, 81, 0)),
					$mdgriffith$elm_ui$Element$Font$size(18)
				]),
			$mdgriffith$elm_ui$Element$text('🎯 Target Section')),
			A2(
			$mdgriffith$elm_ui$Element$paragraph,
			_List_fromArray(
				[
	...
	return A2(
		...
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
				$mdgriffith$elm_ui$Element$htmlAttribute(
				$elm$html$Html$Attributes$class('scroll-container-wrapper'))
			]),
		A2(
			$mdgriffith$elm_ui$Element$el,
			_List_fromArray(
...
var $author$project$Engines$Scroll$Controls$Main$viewContent = function (model) {
	return _List_fromArray(
		[
			...
			_List_fromArray(
				[
					_Utils_Tuple3(0, '📜 Scroll', 'Scroll to target element'),
					_Utils_Tuple3(1, '⏹️ Stop', 'Jump instantly to target position'),
					_Utils_Tuple3(1, '⏸️ Pause', 'Pause scroll at current position'),
					_Utils_Tuple3(1, '▶️ Resume', 'Continue paused scroll'),
					_Utils_Tuple3(1, '⏮️ Reset', 'Jump instantly to start position'),
					_Utils_Tuple3(1, '🔄 Restart', 'Reset to start, then scroll again')
				])),
