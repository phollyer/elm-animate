...
import Element exposing (Element, behindContent, centerX, centerY, column, el, fill, height, html, htmlAttribute, inFront, padding, px, row, spacing, text, width)
...
view model =
    UI.createDocument
        "Transform Order - Keyframes Engine"
        UI.Basic
        (viewContent model)
...
animationArea : Keyframes.AnimState -> Element Msg
...
animatedBox : Keyframes.AnimState -> Permutation -> Element Msg
animatedBox animState perm =
    el
        ...
        (el
            (List.map htmlAttribute (Keyframes.attributes (permutationKey perm) animState)
                ++ [ width (px 80)
                   , height (px 80)
