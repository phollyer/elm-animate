...
We trigger the animation immediately by passing the `initialAnimState` through `Keyframes.animate` using `selectAnimation` to select the first animation to run based on the starting `state`.

??? example "View Source Code"

    ```elm
    --8<-- "docs/examples/src/Concepts/Animate3D/ElmUI/Main.elm:initializeAndTrigger"
    ```
...
??? example "View Source Code"
    ...
    === "Html"
        ...
        ```elm
        --8<-- "docs/examples/src/Concepts/Animate3D/Html/Main.elm:renderCube"
        ```

    === "ElmUI"

        ```elm
        --8<-- "docs/examples/src/Concepts/Animate3D/ElmUI/Main.elm:renderCube"
        ```
...
!!! info "Preventing Event Bubbling with eventsStopPropagation"
    ...
    The solution is `Keyframes.eventsStopPropagation`, which attaches listeners that call `stopPropagation()` on every event. You only need this on elements that have event listeners - elements without listeners simply let events pass through.

    In this example:

    - The **cube** uses `eventsStopPropagation` → forwards events to the handler (detects rotation completion)
    - Each **face** uses `eventsStopPropagation` → only `front-face` forwards to the handler, others send to `NoOp`
    - **Text labels** don't need event handling - they have no listeners, so their events bubble to their parent face

    This ensures each animated element with a listener only triggers events for its own animations.
