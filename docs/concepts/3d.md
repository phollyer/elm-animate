...
## Advanced: Positioning with preserve-3d

Elements with `transform-style: preserve-3d` need explicit CSS positioning for layout to work correctly. Without it, flexbox centering, elm-ui's `centerX`/`centerY`, and their HTML equivalents may have no effect.

### Solutions

**Add `position: relative`** to make standard centering work:
...
??? example "Show Source Code"
    ...
    === "Html"
        ...
        div[
            ...
            [ -- Intermediate container with preserve-3d needs position: relative
            div
...
**Or use transform-based positioning**:
...
??? example "Show Source Code"
    ...
    === "Html"
        ...
        div
            [ View3D.transformStyle View3D.Preserve3D
            , Html.Attributes.style "transform" "translate(100px, 75px)"
            ]
            [ viewCube model ]
        ```
    ...
    === "ElmUI"
        ...
        el
            ...
            , moveRight (toFloat (animAreaWidth - cubeSize) / 2)
            , moveDown (toFloat (animAreaHeight - cubeSize) / 2)
            ]
            (viewCube model)
        ```

        For ElmUI, use the `move*` variants: `moveLeft`, `moveRight`, `moveUp`, `moveDown`.

**Or animate the position** with `Translate.initXY`:
...
??? example "View Source Code"
    ...
    initialAnimState =
        Keyframes.init
            [ Translate.initXY "container" 
                (toFloat (animAreaWidth - cubeSize) / 2) 
                (toFloat (animAreaHeight - cubeSize) / 2)
            ]
    ```
