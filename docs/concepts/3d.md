...
!!! success "Automatic Event Source Detection"
    CSS animation events **bubble up** the DOM tree. When a face's animation ends, the event fires on the face, then bubbles to its parent (the cube).

    **elm-animate automatically handles this** by decoding the source element ID from the CSS animation name. Even when events bubble up, they correctly report which element triggered them.

    In this example:

    - The **cube** has the only event listener via `Keyframes.events`
    - When a **face** or **text** animation ends, it bubbles up to the cube
    - The event correctly reports `Ended "front-face"` or `Ended "front-face-text"` - not `Ended "cube"`
    
    This matches standard DOM behavior where you can identify `event.target`, making nested 3D animations straightforward. Most cases don't need `eventsStopPropagation` - it's only useful when you want to prevent parent handlers from receiving an event at all.
