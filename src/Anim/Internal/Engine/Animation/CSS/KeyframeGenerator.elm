...
generateAnimation : Maybe (List Builder.TransformOrder) -> Builder.IterationCount -> Builder.AnimationDirection -> Maybe Builder.ElementEndStates -> AnimGroupName -> Builder.ProcessedElementConfig -> AnimGroup
generateAnimation maybeOrder iterationCount direction maybeTargets animGroupName processed =
    let
        processedProps =
    ...
    { styles = allStyles
    , animationLayers =
        generateAnimationData maybeOrder maybeTargets animGroupName processedProps
...
generateInitialState : Maybe (List Builder.TransformOrder) -> Builder.IterationCount -> Builder.AnimationDirection -> AnimGroupName -> Builder.ElementConfig -> AnimGroup
generateInitialState maybeOrder iterationCount direction animGroupName elementConfig =
    let
        processed =
    ...
    { styles = allStyles
    , animationLayers =
        generateInitialLayers maybeOrder animGroupName elementConfig.properties
            |> setIterationCount iterationCount
            |> setDirection direction
    , restartCounter = 0
    }
...
generateInitialLayers : Maybe (List Builder.TransformOrder) -> AnimGroupName -> List Builder.PropertyConfig -> List Animation
generateInitialLayers maybeOrder animGroupName properties =
    if List.isEmpty properties then
        ...
        in
        generateAnimationData maybeOrder Nothing animGroupName processed.properties
...
generateAnimationData : Maybe (List Builder.TransformOrder) -> Maybe Builder.ElementEndStates -> AnimGroupName -> List Builder.ProcessedPropertyConfig -> List Animation
generateAnimationData maybeOrder maybeTargets animGroupName processedProps =
...
-}
generateRestartData : Maybe (List Builder.TransformOrder) -> Maybe Builder.ElementEndStates -> AnimGroupName -> String -> List Builder.ProcessedPropertyConfig -> List Animation
generateRestartData maybeOrder maybeTargets animGroupName suffix processedProps =
    if List.isEmpty processedProps then
        []
...
generateRestart : Maybe (List Builder.TransformOrder) -> Builder.IterationCount -> Builder.AnimationDirection -> Maybe Builder.ElementEndStates -> String -> AnimGroupName -> Builder.ProcessedElementConfig -> AnimGroup
generateRestart maybeOrder iterationCount direction maybeTargets suffix animGroupName processed =
    let
        processedProps =
            processed.properties
    ...
    { styles = allStyles
    , animationLayers =
        generateRestartData maybeOrder maybeTargets suffix animGroupName processedProps
            |> setIterationCount iterationCount
            |> setDirection direction
    , restartCounter = 0
    }
