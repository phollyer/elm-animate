...
var $author$project$SmoothMoveScroll$animateToTaskWithConfig = F2(
	function (config, id) {
		var scrollTask = F3(
			function (_v9, _v10, container) {
				var scene = _v9.scene;
				var viewport = _v9.viewport;
				var element = _v10.element;
				var _v3 = function () {
					if (container.$ === 'Nothing') {
						return _Utils_Tuple2(element.x - config.offsetX, element.y - config.offsetY);
					} else {
						var containerInfo = container.a;
						var elementInContainerY = element.y - containerInfo.element.y;
						var targetScrollY = elementInContainerY - config.offsetY;
						var elementInContainerX = element.x - containerInfo.element.x;
						var targetScrollX = elementInContainerX - config.offsetX;
						return _Utils_Tuple2(targetScrollX, targetScrollY);
					}
				}();
				var targetX = _v3.a;
				var targetY = _v3.b;
				var _v5 = function () {
					var minScrollY = 0;
					var minScrollX = 0;
					var maxScrollY = A2($elm$core$Basics$max, 0, scene.height - viewport.height);
					var validY = A2(
						$elm$core$Basics$max,
						minScrollY,
						A2($elm$core$Basics$min, maxScrollY, targetY));
					var maxScrollX = A2($elm$core$Basics$max, 0, scene.width - viewport.width);
					var validX = A2(
						$elm$core$Basics$max,
						minScrollX,
						A2($elm$core$Basics$min, maxScrollX, targetX));
					return _Utils_Tuple2(validX, validY);
				}();
				var clampedX = _v5.a;
				var clampedY = _v5.b;
				var xFrames = A2(
				...
				var xAnimationSteps = A4($author$project$Internal$AnimationCore$animationSteps, xFrames, config.easing, viewport.x, clampedX);
				var yFrames = A2(
					$author$project$SmoothMoveScroll$timingToSpeed,
					config.timing,
					$elm$core$Basics$abs(clampedY - viewport.y));
				var bothAxisFrames = A2(
					$elm$core$Basics$max,
					A2($elm$core$Basics$max, xFrames, yFrames),
					1);
				var xSyncSteps = A4($author$project$Internal$AnimationCore$animationStepsWithFrames, bothAxisFrames, config.easing, viewport.x, clampedX);
				var yAnimationSteps = A2(
					$elm$core$Debug$log,
					'Y Animation Steps',
					A4($author$project$Internal$AnimationCore$animationSteps, yFrames, config.easing, viewport.y, clampedY));
				var ySyncSteps = A2(
					$elm$core$Debug$log,
					'Y Synchronized Steps',
					A4($author$project$Internal$AnimationCore$animationStepsWithFrames, bothAxisFrames, config.easing, viewport.y, clampedY));
				var setViewportTask = function () {
					var _v6 = config.container;
					if (_v6.$ === 'DocumentBody') {
						var _v7 = config.axis;
						switch (_v7.$) {
							case 'X':
							...
							default:
								return $elm$core$Task$sequence(
									A3($elm$core$List$map2, $elm$browser$Browser$Dom$setViewport, xSyncSteps, ySyncSteps));
						}
					} else {
						var containerNodeId = _v6.a;
						var _v8 = config.axis;
						switch (_v8.$) {
							case 'X':
								return $elm$core$Task$sequence(
		...
		var getViewport = function () {
			var _v2 = config.container;
			if (_v2.$ === 'DocumentBody') {
				return $elm$browser$Browser$Dom$getViewport;
			} else {
				var containerNodeId = _v2.a;
		...
		var _v0 = A2($elm$core$Debug$log, 'animateToTaskWithConfig', _Utils_Tuple0);
		return A2(
			$elm$core$Task$andThen,
			...
			case 'ScrollToTop':
				return _Utils_Tuple2(
					...
					A3(
						...
						$author$project$ElmUI$Scroll$ContainerY$Main$NoOp,
...
var $author$project$Common$UI$bulletPoint = function (text_) {
	return A2(
...
var $author$project$Common$UI$contentBlock = F2(
			...
			A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$author$project$Common$UI$bulletPoint('Each block adds to the scrollable height'),
								$author$project$Common$UI$bulletPoint('The gradient background shows scroll position'),
								$author$project$Common$UI$bulletPoint('Smooth scrolling animates between positions')
							]))
					])));
	});
				...
				A2(
					...
					_List_fromArray(
						[
							...
							A2($author$project$Common$UI$contentBlock, 1, 'This is content block 1. Each block adds to the scrollable height and demonstrates ElmUI styling.'),
							A2($author$project$Common$UI$contentBlock, 2, 'Content block 2 continues the gradient transition from white to dark with ElmUI elements.'),
							A2($author$project$Common$UI$contentBlock, 3, 'Content block 3 shows the middle section of our scrollable content using ElmUI layout.'),
							A2(
							$mdgriffith$elm_ui$Element$el,
							_List_fromArray(
								[
							...
							A2(
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$author$project$Common$UI$bulletPoint('This block serves as the middle anchor point'),
												$author$project$Common$UI$bulletPoint('The gradient background shows scroll position'),
												$author$project$Common$UI$bulletPoint('Smooth scrolling animates between positions')
											]))
									]))),
							A2($author$project$Common$UI$contentBlock, 5, 'Content block 5 continues toward the bottom of the container with ElmUI.'),
							A2($author$project$Common$UI$contentBlock, 6, 'Content block 6 shows we\'re getting closer to the bottom using ElmUI layout.'),
							A2($author$project$Common$UI$contentBlock, 7, 'Content block 7 is near the end with darker background colors in ElmUI.'),
							A2($author$project$Common$UI$contentBlock, 8, 'Content block 8 is almost at the bottom of the scrollable ElmUI content.'),
							A2(
							$mdgriffith$elm_ui$Element$el,
							_List_fromArray(
								[
