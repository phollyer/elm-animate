...
{
	if (region.bl.aC === region.bz.aC)
	{
		return 'on line ' + region.bl.aC;
	}
	return 'on lines ' + region.bl.aC + ' through ' + region.bz.aC;
}
...
var $author$project$Scroll$Sub$FirstScroll$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{ak: $author$project$Scroll$Engine$Sub$init, aE: $author$project$Scroll$Sub$FirstScroll$Main$Idle},
							...
							A3(
								...
								F2(
									function (viewport, containerEl) {
										return {aO: containerEl, a1: $elm$core$Maybe$Nothing, s: viewport};
									}),
								$author$project$Scroll$Internal$Shared$Dom$getViewport(container),
								$author$project$Scroll$Internal$Shared$Dom$getContainerInfo(container)));
					}
...
var $author$project$Scroll$Internal$Engine$Sub$createScrollAnimationFromDom = F4(
	function (scrollBuilder, scrollTarget, domResult, element) {
		...
		var elementContentPosition = function () {
			var _v3 = domResult.aO;
			if (!_v3.$) {
				var containerEl = _v3.a;
				return {d: ((element.ar.d - containerEl.ar.d) + viewport.s.d) - offsetX, e: ((element.ar.e - containerEl.ar.e) + viewport.s.e) - offsetY};
			} else {
				return {d: (element.ar.d + viewport.s.d) - offsetX, e: (element.ar.e + viewport.s.e) - offsetY};
			}
		}();
		...
		return {c: config, L: startPosition.d, M: startPosition.e, ad: delayMs === 0.0, aq: delayMs, aT: actualDuration, E: 0.0, y: false, W: 0.0, B: startPosition.d, C: startPosition.e};
	});
	...
	function (deltaMs, animation) {
		if (!animation.ad) {
			...
			return (_Utils_cmp(newElapsedMs, animation.aq) > -1) ? _Utils_update(
				animation,
				{ad: true, E: 0.0}) : _Utils_update(
				animation,
				{E: newElapsedMs});
		} else {
			var newElapsedMs = animation.E + deltaMs;
			var progress = A2($elm$core$Basics$min, 1.0, newElapsedMs / animation.aT);
			...
			var newCurrentX = function () {
				...
				if (_v1 === 1) {
					...
					return animation.B + ((animation.c.aw - animation.B) * easedProgress);
				}
			}();
			var newCurrentY = function () {
				...
				if (!_v0) {
					...
					return animation.C + ((animation.c.ax - animation.C) * easedProgress);
				}
			}();
			return _Utils_update(
				animation,
				{L: newCurrentX, M: newCurrentY, E: newElapsedMs, W: progress});
		}
	});
	...
	function (fromInternalEvent, toMsg, msg, _v0) {
		...
		switch (msg.$) {
						...
						function (scrollId, scrollAnim, _v3) {
							...
							if (scrollAnim.y) {
								...
								var scrollCmd = A2(
									...
									A3($author$project$Scroll$Internal$Shared$Dom$setViewport, updatedScroll.c.h, updatedScroll.L, updatedScroll.M));
								...
								var progressEvent = A3(
									...
									cid,
									{d: updatedScroll.L, e: updatedScroll.M},
									updatedScroll.W);
								if (updatedScroll.W < 1.0) {
									return _Utils_Tuple3(
			...
			case 1:
				...
				var hasDistance = A5($author$project$Scroll$Internal$Engine$Sub$calculateDistance, scrollAnimation.c.aa, scrollAnimation.B, scrollAnimation.C, scrollAnimation.c.aw, scrollAnimation.c.ax) > 0;
		...
		if (!msg.$) {
			...
			var updatedModel = A2(
				...
				_Utils_update(
					...
					{ak: newScrollState}),
				events);
			return _Utils_Tuple2(updatedModel, scrollCmd);
		}
	});
	...
	var _v0 = function () {
		switch (status.$) {
			...
			case 2:
				...
				return _Utils_Tuple2(
					'#3b82f6',
					'Progress... ' + ($elm$core$String$fromFloat(progress * 100) + '%'));
			default:
				var err = status.a;
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Scroll$Sub$FirstScroll$Main$statusBar(model.aE),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
...
var $author$project$Scroll$Sub$FirstScroll$Main$main = $elm$browser$Browser$element(
	{dh: $author$project$Scroll$Sub$FirstScroll$Main$init, d3: $author$project$Scroll$Sub$FirstScroll$Main$subscriptions, er: $author$project$Scroll$Sub$FirstScroll$Main$update, es: $author$project$Scroll$Sub$FirstScroll$Main$view});
